...
function getHomeNovels(req, res) {
    novels_model.sequelize.query('SELECT n.*, ROUND((SELECT AVG(rate_value) FROM novels_ratings where novel_id = n.id), 1) as nvl_rating, (SELECT COUNT(id) FROM novels_ratings WHERE novel_id = n.id) as nvl_ratings_count FROM novels n WHERE n.nvl_status IN ("Active", "Finished") ORDER BY nvl_ratings_count desc LIMIT 15;', { type: novels_model.sequelize.QueryTypes.SELECT })
        .then(topNovels => {
...
function getChapter(req, res) {
    const id = req.params.id;
    // chapters_model.sequelize.query('SELECT *, (SELECT nvl_title FROM novels n WHERE n.id = c.nvl_id) AS nvl_title, (SELECT nvl_name FROM novels n WHERE n.id = c.nvl_id) AS nvl_name, (SELECT user_login FROM users u WHERE u.id = c.chp_author) AS user_login, IFNULL((SELECT JSON_ARRAYAGG(JSON_OBJECT("id", cms.id, "comment_content", cms.comment_content, "replys_count", (SELECT COUNT(cr.id) FROM replys cr WHERE cr.comment_id = cms.id), "user_id", cms.user_id, "user_login", (SELECT user_login FROM users u WHERE u.id = cms.user_id), "image", (SELECT image FROM users u WHERE u.id = cms.user_id), "createdAt", cms.createdAt, "updatedAt", cms.updatedAt, "likes_count", (SELECT COUNT(id) FROM likes l WHERE l.comment_id = cms.id), "likes", IFNULL((SELECT JSON_ARRAYAGG(JSON_OBJECT("id", l.id, "comment_id", l.comment_id, "user_id", l.user_id, "user_login", (SELECT user_login FROM users u WHERE u.id = l.user_id))) FROM likes l WHERE l.comment_id = cms.id), JSON_ARRAY()))) FROM comments cms WHERE cms.chp_id = c.id), JSON_ARRAY()) AS comments FROM chapters c WHERE c.id = ? AND c.chp_status="Active"', { replacements: [id], type: chapters_model.sequelize.QueryTypes.SELECT })
    chapters_model.sequelize.query('SELECT *, (SELECT nvl_title FROM novels n WHERE n.id = c.nvl_id) AS nvl_title, (SELECT nvl_name FROM novels n WHERE n.id = c.nvl_id) AS nvl_name, (SELECT user_login FROM users u WHERE u.id = c.chp_author) AS user_login, (SELECT COUNT(rc.id) FROM reaction_chapters rc WHERE rc.chapter_id=c.id) AS reactions_count, IFNULL( (SELECT JSON_ARRAYAGG( JSON_OBJECT( "id", cms.id, "comment_content", cms.comment_content, "replys_count", (SELECT COUNT(cr.id) FROM replys cr WHERE cr.comment_id = cms.id), "user_id", cms.user_id, "user_login", (SELECT user_login FROM users u WHERE u.id = cms.user_id), "image", 	(SELECT image FROM users u WHERE u.id = cms.user_id), "createdAt", cms.createdAt, "updatedAt", cms.updatedAt, "likes_count", 	(SELECT COUNT(id) FROM likes l WHERE l.comment_id = cms.id), "likes", 	IFNULL( (SELECT JSON_ARRAYAGG( JSON_OBJECT( "id", l.id, "comment_id", l.comment_id, "user_id", l.user_id, "user_login",	 (SELECT user_login FROM users u WHERE u.id = l.user_id) ) ) FROM likes l WHERE l.comment_id = cms.id), JSON_ARRAY() ) ) )	 FROM comments cms WHERE cms.chp_id = c.id), JSON_ARRAY() ) AS comments,	 IFNULL( (SELECT JSON_ARRAYAGG( JSON_OBJECT( "id", rc.id, "user_id", rc.user_id, "chapter_id", rc.chapter_id , "reaction_id", rc.reaction_id ) )	FROM reaction_chapters rc WHERE rc.chapter_id = c.id), JSON_ARRAY() ) AS reactions, 	 IFNULL( (SELECT JSON_ARRAYAGG( JSON_OBJECT( "reaction_id", r.id, "reaction_name", r.reaction_name, "reaction_count",IFNULL((SELECT COUNT(rc.reaction_id) FROM reaction_chapters rc WHERE rc.chapter_id = c.id AND rc.reaction_id = r.id), 0) ) )FROM  reaction r ), JSON_ARRAY() )  AS total_reactions FROM chapters c WHERE c.id = ? AND c.chp_status = "Active";', { replacements: [id], type: chapters_model.sequelize.QueryTypes.SELECT })
        .then(chapter => {
            if (chapter.length > 0) {
                chapter = mariadbHelper.verifyJSON(chapter, ['comments', 'reactions', 'total_reactions' ]);
