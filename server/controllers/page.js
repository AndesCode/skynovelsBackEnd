...
function getAdvertisement(req, res) {
    const id = req.params.id;
    advertisements_model.sequelize.query('SELECT a.*, IFNULL((SELECT CONVERT(CONCAT("[", GROUP_CONCAT(JSON_OBJECT("id", l.id, "adv_id", l.adv_id, "user_id", l.user_id, "user_login", (SELECT user_login FROM users u where u.id = l.user_id))), "]"), JSON) FROM likes l where l.adv_id = a.id), CONVERT(CONCAT("[]"), JSON)) AS likes, (SELECT user_login FROM users u WHERE u.id = a.user_id) AS user_login, IFNULL((SELECT CONVERT(CONCAT("[", GROUP_CONCAT(JSON_OBJECT("id", c.id, "comment_content", c.comment_content, "user_id", c.user_id, "user_login", (SELECT user_login FROM users u where u.id = c.user_id), "user_profile_image", (SELECT user_profile_image FROM users u where u.id = c.user_id), "likes_count", (SELECT COUNT(id) FROM likes l where l.comment_id = c.id), "replys_count", (SELECT COUNT(id) FROM replys r where r.comment_id = c.id), "likes", (IFNULL((SELECT CONVERT(CONCAT("[", GROUP_CONCAT(JSON_OBJECT("id", l.id, "comment_id", l.comment_id, "user_id", l.user_id, "user_login", (SELECT user_login FROM users u where u.id = l.user_id))), "]"), JSON) FROM likes l where l.comment_id = c.id), CONVERT(CONCAT("[]"), JSON))))), "]"), JSON) FROM comments c where c.adv_id = a.id), CONVERT(CONCAT("[]"), JSON)) as comments FROM advertisements a WHERE a.id = ? AND a.adv_img IS NOT NULL', { replacements: [id], type: advertisements_model.sequelize.QueryTypes.SELECT })
        .then(advertisements => {
